(function() {
    ...
    var getTypeDefinition = function*(ctor, typesService) {
        return yield* typesService.getTypeDefinitionFromCtor(ctor);
    };



    var getTypeDefinitionFromObject = function*(record, ctor, typesService) {
        return yield* typesService.getTypeDefinitionFromObject(record, ctor);
    };



    var validate = function*(record, ctor, typesService) {
        var typeDefinition = yield* getTypeDefinitionFromObject(record, ctor, typesService);
    ...
    var findById = function*(ctor, id, context) {
        var query = context.db.setRowId({}, id);
        return yield* findOne(ctor, query, {}, context);
    };



    var find = function*(ctor, query, options, context) {
        ...
        var results;
        var typeDefinition = yield* getTypeDefinition(ctor, typesService);
    ...
    var findOne = function*(ctor, query) {
        var options, context;
        ...
        typeDefinition = yield* getTypeDefinition(ctor, typesService);
    ...
    var count = function*(ctor, query, context) {
        var db = context.db, typesService = context.typesService;
    ...
    var destroyAll = function*(ctor, query, context) {
        var db = context.db, typesService = context.typesService;
    ...
    var save = function*(record, ctor, context) {
        var db = context.db, typesService = context.typesService;
        ...
        var errors = yield* validate(record, ctor, typesService);

        if (!errors.length) {
            if (db.getRowId(record) && (typeDefinition.concurrency === 'optimistic' || !typeDefinition.concurrency)) {
                var _item = yield* findById(ctor, db.getRowId(record), context);
    ...
    var destroy = function*(record, ctor, context) {
        var db = context.db, typesService = context.typesService;
    ...
    var link = function*(record, ctor, name, context) {
        var db = context.db, typesService = context.typesService;

        var query, result, _i, _j;

        var typeDef = yield* getTypeDefinitionFromObject(record, ctor, typesService);
        ...
        if (link.key) {
            ...
            if (typeof link.key === 'string') {
                switch (typeDef.schema.properties[link.key].type) {
                    case 'string':
                        return yield* findById(ctor, record[link.key], context);
                    case 'array':
