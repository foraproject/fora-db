(function() {
    ...
    var find = function*(typeDefinition, query, options, context) {
        ...
        if (items.length) {
            ...
            for (var _i = 0; _i < items.length; _i++) {
                var item = items[_i];
                results.push(yield* typesService.constructModel(item, typeDefinition, { systemFields: ["__updateTimestamp", "__shard"] }));
            }
            return results;
        } else {
            return [];
    ...
    var findOne = function*(typeDefinition, query) {
        ...
        if (result) {
            return yield* typesService.constructModel(result, typeDefinition, { systemFields: ["__updateTimestamp", "__shard"] });
        }
    };
    ...
    var save = function*(record, typeDefinition, context, options) {
        var db = context.db, typesService = context.typesService;
        ...
        if (!errors.length) {
            if (db.getRowId(record) && (typeDefinition.concurrency === 'optimistic' || !typeDefinition.concurrency)) {
                var _item = yield* db.findOne(typeDefinition, db.setRowId({}, db.getRowId(record)));
            ...
            if (!db.getRowId(record)) {
                ...
                var insertedObject = yield* db.insert(typeDefinition, record);
                if (!options || !options.modifyCurrent) {
                    result = yield* typesService.constructModel(insertedObject, typeDefinition, { systemFields: ["__updateTimestamp", "__shard"] });
                } else {
                    db.setRowId(record, db.getRowId(insertedObject));
                    result = record;
                }
            } else {
                ...
                yield* db.update(typeDefinition, query, record);
                result = record;
            }
            return result;
